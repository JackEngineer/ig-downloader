...
describe("ConfigManager", () => {
  function withConfig<T>(
    fn: (config: ConfigManager, tempDir: string) => Promise<T>,
  ): () => Promise<T> {
    return async () => {
  ...
  describe("save", () => {
    it(
      ...
      withConfig(async (config, tempDir) => {
  ...
  describe("addUser", () => {
    ...
    it(
      "should normalize username (remove @ and lowercase)",
      withConfig(async (config) => {
        const user = config.addUser("@TestUser");
        expect(user?.username).toBe("testuser");
      }),
    ...
    it(
      "should return null for duplicate user",
      withConfig(async (config) => {
  ...
  describe("getEnabledUsers", () => {
    it(
      "should return only enabled users",
      withConfig(async (config) => {
